...
data End = L | R deriving (Eq, Show, Read)
type Scores = (Int, Int) -- P1’s score, P2’s score
type MoveNum = Int
type Hand = [Domino]
type DomsPlayer = Hand -> DominoBoard -> Player -> Scores -> IO (Domino, End)
...
domsMatch :: DomsPlayer -> DomsPlayer -> Int -> Int -> IO (Int, Int)
domsMatch p1 p2 n seed =
    domsGames p1 p2 n (mkStdGen seed) (0, 0)
  where
    domsGames p1 p2 0 gen wins = pure wins
    domsGames p1 p2 n gen (p1_wins, p2_wins) = do
        winner <- playGame p1 p2 (if odd n then P1 else P2) gen1
        if winner == P1
            then domsGames p1 p2 (n -1) gen2 (p1_wins + 1, p2_wins)
            else domsGames p1 p2 (n -1) gen2 (p1_wins, p2_wins + 1)
      where
        (gen1, gen2) = split gen
...
{- playGame: play a single game (where winner is determined by a player reaching
 ...
 -}
-- FIXME: Should I be using `return` instead of `pure` here?
playGame :: DomsPlayer -> DomsPlayer -> Player -> StdGen -> IO Player
playGame p1 p2 firstPlayer gen =
    playGame' p1 p2 firstPlayer gen (pure (0, 0))
  where
    playGame' p1 p2 firstPlayer gen scores = do
        ss <- scores
        let newScores = playDomsRound p1 p2 firstPlayer currentG ss
            (currentG, nextG) = split gen
        case ss of
            (61, _) -> pure P1
            (_, 61) -> pure P2
            _ -> playGame' p1 p2 (if firstPlayer == P1 then P2 else P1) nextG newScores
...
playDomsRound :: DomsPlayer -> DomsPlayer -> Player -> StdGen -> (Int, Int) -> IO (Int, Int)
playDomsRound p1 p2 first gen scores =
  ...
  where
    ...
    playDomsRound' _ _ _ (_, _, _, scores@(61, _)) = pure scores
    playDomsRound' _ _ _ (_, _, _, scores@(_, 61)) = pure scores
    playDomsRound' p1 p2 turn gameState@(hand1, hand2, board, (score1, score2))
        | p1_blocked && p2_blocked = pure (score1, score2)
        | turn == P1 && p1_blocked = playDomsRound' p1 p2 P2 gameState
        | turn == P2 && p2_blocked = playDomsRound' p1 p2 P1 gameState
        | otherwise = do
            (domino, end) <- (if turn == P1 then p1 hand1 else p2 hand2) board turn (score1, score2)
            let nextPlayer = if turn == P1 then P2 else P1
                ...
                s1 = score1 + score
                s2 = score2 + score
            playDomsRound' p1 p2 nextPlayer newGameState
      where
...
randomPlayer (domino : rest) board player scores
    | leftBoard /= Nothing = pure (domino, L)
    | rightBoard /= Nothing = pure (domino, R)
